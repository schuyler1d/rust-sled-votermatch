...
fn run(
    ...
    let mut byte = || [rng.gen::<u64>()].as_byte_slice().unwrap().to_vec();
    let mut rng = thread_rng();
    let mut valbytes = || {
        let randsize = rng.gen_range(0,32);
        let randvec : Vec<u8> = (0..randsize).map(|_| rng.gen::<u8>()).collect();
        let vec = Vec::<u8>::with_capacity(randsize);
        let my_array: [u8; 8] = rng.gen();
        randvec //my_array
    };
    //let mut byte = || vec![rng.gen::<u8>(), rng.gen::<u8>(), rng.gen::<u8>(), rng.gen::<u8>(),
